...
export class CatalogPage extends MainPage {
    ...
    async getCatalogText(): Promise<string> {
        ...
        let linktoCatalog = await this.findElement(
            By.xpath(this.linktoCatalog),
            10000
        );
        await linktoCatalog.click();
        const buttonOnPopup = await this.findElement(
            By.css(this.buttonOnPopup)
        );
        await buttonOnPopup.click();
        const textCatalogOnPage = await this.findElement(
            By.css(this.textCatalogOnPage)
        );
