...
export class LogInPage extends BasePage {
    private loginButtonOnLoginPage: string =
    ...
    private errorMessageForEmptyLogIn: string =
        "//div[contains(@class, 'auth-form__description') and contains(text(), 'Укажите ник или e-mail')]";
    private errorMessageForEmptyPassword: string =
    ...
    async getUnsuccessfulLoginErrors(): Promise<{
        logInError: string;
        passwordError: string;
    }> {
        const loginButtonOnLoginPage = await this.driver.findElement(
            By.xpath(this.loginButtonOnLoginPage)
        );
        await loginButtonOnLoginPage.click();

        const errorMessageForEmptyLogIn = await this.driver.wait(
            until.elementLocated(By.xpath(this.errorMessageForEmptyLogIn))
        );
        const errorMessageTextForEmptyLogIn =
            await errorMessageForEmptyLogIn.getText();

        const errorMessageForEmptyPassword = await this.driver.findElement(
            By.xpath(this.errorMessageForEmptyPassword)
        );
